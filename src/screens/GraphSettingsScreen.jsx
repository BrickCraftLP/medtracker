...
export default function GraphSettingsScreen() {
  const navigate = useNavigate()
  const { t } = useLanguage()
  const { gridLines, smoothLines, showDots, heatmapTheme, chartColor, setSetting } = useGraphSettings()
  ...
  return (
    <div className="scroll-container" style={{ background: 'var(--bg-primary)' }}>
      <div style={{ padding: '16px 16px 60px' }}>
              ...
              {Object.keys(COLOR_PALETTES).map(pal => (
                <motion.button
                  ...
                  style={{
                    ...
                    padding: '7px 4px',
                    borderRadius: 12,
                    border: `1.5px solid ${colorPalette === pal ? chartColor : 'var(--border)'}`,
                    background: colorPalette === pal ? chartColor + '18' : 'var(--bg-tertiary)',
                    cursor: 'pointer',
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: 'center',
                ...
                >
                  ...
                  <span style={{
                    ...
                    color: colorPalette === pal ? chartColor : 'var(--text-secondary)',
                  }}>
                    {t(PAL_KEY[pal])}
                  </span>
                </motion.button>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {COLOR_PALETTES[colorPalette].map(c => (
                <motion.button
                  key={c}
                  whileTap={{ scale: 0.85 }}
                  onClick={() => setSetting('chartColor', c)}
                  style={{
                    width: 44,
                    height: 44,
                    borderRadius: 12,
                    background: c,
                    border: chartColor === c ? `2px solid var(--text-primary)` : '2px solid rgba(255,255,255,0.1)',
                    cursor: 'pointer',
                    boxShadow: chartColor === c ? `0 0 0 2px var(--bg-primary), 0 0 0 4px ${c}` : 'none',
                    transition: 'all 0.15s',
                  }}
                />
              ))}
        ...
        {/* Heatmap section */}
        <Section title={t('settings.heatmapSection')}>
          <div style={{ padding: '16px 16px', display: 'flex', justifyContent: 'space-around', alignItems: 'flex-end' }}>
            <ColorSwatch
              color="blue"
              label={t('settings.theme.blue')}
              selected={heatmapTheme === 'blue'}
              onClick={() => setSetting('heatmapTheme', 'blue')}
            />
            <ColorSwatch
              color="green"
              label={t('settings.theme.green')}
              selected={heatmapTheme === 'green'}
              onClick={() => setSetting('heatmapTheme', 'green')}
            />
            <ColorSwatch
              color="purple"
              label={t('settings.theme.purple')}
              selected={heatmapTheme === 'purple'}
              onClick={() => setSetting('heatmapTheme', 'purple')}
            />
          </div>
        </Section>
